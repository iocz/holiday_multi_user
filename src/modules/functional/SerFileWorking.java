...
public class SerFileWorking implements DataSaveLoad {
    ...
    public void loadAllEN() throws ClassNotFoundException, IOException, JDOMException, ParseException {
        MainClass.holidays = serSaveLoad.loadHoliday(HOLIDAY_PATCH_EN);
        MainClass.countries = serSaveLoad.loadCountry(COUNTRY_PATCH_EN);
        MainClass.traditions = serSaveLoad.loadTradition(TRADITION_PATCH_EN);

    }

    public void loadAllRU() throws ClassNotFoundException, IOException, JDOMException, ParseException {
        MainClass.holidays = serSaveLoad.loadHoliday(HOLIDAY_PATCH_RU);
        MainClass.countries = serSaveLoad.loadCountry(COUNTRY_PATCH_RU);
        MainClass.traditions = serSaveLoad.loadTradition(TRADITION_PATCH_RU);
    }

    public void saveAllEN() throws IOException {
        serSaveLoad.saveHolidays(MainClass.holidays, HOLIDAY_PATCH_EN);
        serSaveLoad.saveCountry(MainClass.countries, COUNTRY_PATCH_EN);
        serSaveLoad.saveTradition(MainClass.traditions, TRADITION_PATCH_EN);
    }

    public void saveAllRU() throws IOException {
        serSaveLoad.saveHolidays(MainClass.holidays, HOLIDAY_PATCH_RU);
        serSaveLoad.saveCountry(MainClass.countries, COUNTRY_PATCH_RU);
        serSaveLoad.saveTradition(MainClass.traditions, TRADITION_PATCH_RU);
    }
}